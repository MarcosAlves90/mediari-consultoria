      ...
      <div class="flex flex-col items-center justify-center py-1">
        ...
        <div v-if="!modelValue" class="text-sm text-primary-text">
          <span class="font-medium">{{
            t('careers.form.file_upload.click_to_upload')
          }}</span>
          <span class="text-secondary-text">{{
            t('careers.form.file_upload.drag_file_here')
          }}</span>
        </div>
...
<style scoped>
  ...
  .file-upload-area.file-selected {
    box-shadow:
      0 0 0 1px var(--color-accent-color),
      0 2px 8px rgba(0, 0, 0, 0.15);
    background: linear-gradient(
      135deg,
      var(--color-accent-color),
      rgba(var(--color-accent-color-rgb), 0.8)
    );
  }
  ...
  .file-upload-area.dragover {
    ...
    box-shadow:
      0 0 0 3px rgba(var(--color-accent-color-rgb), 0.1),
