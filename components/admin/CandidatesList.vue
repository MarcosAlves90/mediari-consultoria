<template>
    <div class="bg-body-bg-dark rounded-lg shadow border border-gray-200">
        <div class="p-1 border-b border-gray-200">
            ...
            <div class="mt-0.75 space-y-0.75">
                <input
                    ...
                    class="w-full px-0.75 py-0.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-accent-color focus:border-accent-color text-sm"
                />

                <select
                    v-model="selectedArea"
                    class="w-full px-0.75 py-0.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-accent-color focus:border-accent-color text-sm bg-body-bg-dark"
                >
        ...
        <!-- Candidates List -->
        <div v-else class="divide-y divide-gray-200 max-h-24 overflow-y-auto">
            <div
                v-for="candidate in filteredCandidates"
                :key="candidate.id"
                @click="$emit('select', candidate)"
                :class="[
                    'p-1 cursor-pointer hover:bg-gray-50 transition-colors',
                    selectedCandidate?.id === candidate.id ? 'bg-accent-color/10 border-r-1 border-accent-color' : ''
                ]"
            >
