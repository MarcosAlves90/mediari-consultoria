...
export default defineEventHandler(async (event) => {
  ...
  if (!idToken) {
    return sendError(event, createError({ statusCode: 400, statusMessage: 'idToken obrigatório' }))
  }

  const admin = initFirebaseAdmin()
  if (!admin) {
    return sendError(event, createError({ statusCode: 500, statusMessage: 'falha na inicialização do firebase-admin' }))
  }
  ...
  try {
    ...
    return sendError(event, createError({ statusCode: 401, statusMessage: 'idToken inválido' }))
  }
})
