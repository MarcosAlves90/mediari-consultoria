...
defineOgImageComponent('Mediari', {
    title: (props.errorMessage?.statusCode ?? '') + ' ' + t('error.title')
})
...
<template>
    <div
        ...
        <div class="header flex items-center gap-1">
            ...
            <h1 class="text-[5rem] m-0">{{ props.errorMessage.statusCode }}</h1>
        </div>
        <p class="error-page__p" v-if="props.errorMessage.statusCode === 404">{{ t('error.not_found') }}</p>
