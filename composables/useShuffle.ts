...
export function createShuffle(config?: { storageKeyPrefix?: string }) {
    ...
    return {
        getShuffled(groups: Array<Record<string, string>>, namespace = 'default', forceNew = false) {
            if (typeof window === 'undefined') {
                // SSR: devolver ordem natural para evitar mismatch
                return (groups || []).map(g => ['A','B','C','D'].map(k => ({ key: k, label: g[k] })));
            }
        ...
        regenerateSeed(groups: Array<Record<string, string>>, namespace = 'default') {
